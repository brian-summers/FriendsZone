...
| Themes | Real. Three palettes - **Verdigris**, **Harbor**, and **Signal** - each in light and dark, chosen independently in **Settings**. Signal is built for colour vision deficiency and its hues are measured against simulated protanopia, deuteranopia and tritanopia in CI |
| Full day | Real. The week grid draws **00:00 to 24:00**. Hours and minutes are separate fields, so any quarter hour is two short lists away |
| Unavailable hours | Real. One recurring daily window, shaded on the grid, in which friends cannot propose plans. It is not an event: no name, never busy, and the server refuses proposals inside it ([ADR 0030](docs/adr/0030-full-day-grid-and-quiet-hours.md)) |
| Messages | Real. A mailbox between friends: conversations, unread counts, a thread. **No read receipts** - the sender is never told you read it ([ADR 0029](docs/adr/0029-direct-messages-and-discoverability.md)) |
| Being findable | Real. Choose **anyone**, **only my exact handle**, or **nobody**. Opting out is indistinguishable from not existing, and nobody is told what you chose |
| Finding people | Real. Search by handle or display name on the **People** screen (its own tab, not buried in Settings). Bounded to 20 results with a two-character minimum, and someone in a block relationship with you simply is not there ([ADR 0028](docs/adr/0028-friend-requests-and-blocking.md)) |
